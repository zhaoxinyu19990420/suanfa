...
class Ant(object):
    ...
    def __clean_data(self):
        ...
        self.open_table_city = [True for i in range(city_num)]  # 探索城市的状态，布尔变量类型，列表，true代表还未探索，false已经探索

        city_index = random.randint(0, city_num - 1)  # 随机初始出生点
        self.current_city = city_index
        self.path.append(city_index)
        self.open_table_city[city_index] = False #初始化已经访问过初始城市
        self.move_count = 1 #当move_count等于城市数量时，代表已经遍历所有城市
    ...
    def __choice_next_city(self):
        ...
        for i in range(city_num):
            if self.open_table_city[i]:  #若可以访问，计算概率
                try:
